...
1. **OAuth SSO 必须人工点"同意"一次**：第一次登录无法绕过（`--login` 时弹真浏览器让用户操作）
2. **多维表格 ≥ 1000 行需要滚动加载**：单次抓取可能不全
3. **钉钉前端升级可能破坏 selector**：定期更新
4. **法律**：仅限抓取你**有合法访问权**的内容（自己的文档/加入的圈子/付费订阅的内容）。不要用于盗取付费内容、爬取他人非公开数据

## 工作模式

- **默认**：新版 Chromium headless + stealth（注入 navigator.webdriver=undefined）→ 完全后台跑，**无 Dock 弹窗、无 Mission Control、无 Cmd+Tab**
- **`--login` 或 `--headed`**：可见 headed 模式（首次登录或调试用）
